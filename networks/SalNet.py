...
class SALNetwork(nn.Module):
    def __init__(self,conf,latent_size):
        super().__init__()
        self.decoder = Decoder(latent_size=latent_size,**conf)
        self.encoder = SimplePointnet_VAE(hidden_dim=2*latent_size,c_dim=latent_size)
        self.decode_mnfld_pnts = conf['decode_mnfld_pnts']
    ...
    def forward(self, non_mnfld_pnts, mnfld_pnts):
        ...
        if self.decode_mnfld_pnts:
            mnfld_pnts = torch.cat([latent.unsqueeze(1).repeat(1, mnfld_pnts.shape[1], 1),
                                    mnfld_pnts], dim=-1)
