...
from networks.SalNet import SALNetwork
from experiments.SAL_FAMILY.train_program import train, reconstruct
...
def run(args, exper_specs):
    ...
    # network setting
    net = SALNetwork(conf=exper_specs['NetworkSpecs'],
                     latent_size=exper_specs['CodeLength']).cuda()

    # optimizer setting
    lr_schedules = get_lr(lr_config=exper_specs['LearningRateSchedule'])
    optimizer = torch.optim.Adam(params=net.parameters(), lr=lr_schedules[0].get_learning_rate(0))
...
if __name__ == '__main__':
    ...
    exp_etc = '/home/syao/Program/Source/New3D/experiments/AE_IF/experiments_config/dfaust_sal.json'
